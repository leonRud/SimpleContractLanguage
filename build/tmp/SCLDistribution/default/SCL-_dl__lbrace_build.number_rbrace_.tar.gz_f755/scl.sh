...
# Try (in order): $SCL_JDK, .../scl.jdk, .../jbr, .../jbr-x86, $JDK_HOME, $JAVA_HOME, "java" in $PATH.
# ---------------------------------------------------------------------
IDEA_VENDOR_NAME='null'
# shellcheck disable=SC2154
if [ -n "$SCL_JDK" ] && [ -x "$SCL_JDK/bin/java" ]; then
  JDK="$SCL_JDK"
fi

if [ -z "$JDK" ] && [ -s "${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/SCL1.0/scl.jdk" ]; then
  USER_JRE=$("$CAT" "${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/SCL1.0/scl.jdk")
  if [ ! -d "$USER_JRE" ]; then
...
if [ -z "$JDK" ] || [ ! -x "$JAVA_BIN" ]; then
  ...
  message "No JDK found. Please validate either SCL_JDK, JDK_HOME or JAVA_HOME environment variable points to valid JDK installation."
  fi
  exit 1
fi
...
# ---------------------------------------------------------------------
# shellcheck disable=SC2154
if [ -n "$SCL_PROPERTIES" ]; then
  IDE_PROPERTIES_PROPERTY="-Didea.properties.file=$SCL_PROPERTIES"
fi
...
elif [ -r "${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/SCL1.0/scl$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/SCL1.0/scl$BITS.vmoptions"
elif [ -r "$IDE_BIN_HOME/scl$BITS.vmoptions" ]; then
  # default, standard installation
  VM_OPTIONS_FILE="$IDE_BIN_HOME/scl$BITS.vmoptions"
else
  # default, universal package
  test "$OS_TYPE" = "Darwin" && OS_SPECIFIC="mac" || OS_SPECIFIC="linux"
  VM_OPTIONS_FILE="$IDE_BIN_HOME/$OS_SPECIFIC/scl$BITS.vmoptions"
fi
...
# shellcheck disable=SC2154
if [ -n "$SCL_CLASSPATH" ]; then
  CLASSPATH="$CLASSPATH:$SCL_CLASSPATH"
fi
...
IDEA_PATHS_SELECTOR=SCL1.0
"$JAVA_BIN" \
  ${AGENT} \
  -classpath "$CLASSPATH" \
  ${VM_OPTIONS} \
  "-XX:ErrorFile=$HOME/java_error_in_SCL_%p.log" \
  "-XX:HeapDumpPath=$HOME/java_error_in_SCL.hprof" \
